...
const FormUserRegister: React.FC<RegisterProps> = ({ verifyAccess }) => {
    ...
    const submit = (data: object) => {
        ...
        axios.post("http://localhost:7500/api/v1/user-register/create", data)
            .then(res => console.log(res.data))
    }

    return (
        <div className='static'>
            <form className="h-heightFormRegister w-widthFormRegister bg-customBgRegister absolute top-20 right-0"
                    ...
                    <div className=" h-auto w-6/12 px-6">
                        <section className="my-4">
                            ...
                            <input type="text" id="name"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800" {...register("name")} required />
                        </section>
                        <section className="my-4">
                            <label htmlFor="email">Mail *</label >
                            <input type="email" id="email"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800" {...register("email")} required />
                        </section>
                        <section className="my-4">
                            <label htmlFor="pass">Contraseña *</label >
                            <input type="password" id="pass"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800"  {...register("password")} required />
                        </section>
                        <section className="my-4">
                            <label htmlFor="address">Dirección</label >
                            <input type="text" id="address"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800" {...register("city")} />
                        </section>
                    </div>
                    <div className="h-auto w-6/12 px-6">
                        <section className="my-4">
                            ...
                            <input type="text" id="lastName"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800" {...register("lastName")} required />
                        </section>
                        <section className="my-4">
                            <label htmlFor="phone">Teléfono</label >
                            <input type="number" id="phone" inputMode="numeric"
                                className="w-full rounded-lg bg-white h-7 border border-gray-800" {...register("phone")} />
                        </section>
                        <section className="my-4">
